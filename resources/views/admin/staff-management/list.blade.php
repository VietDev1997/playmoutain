    ...
    <div class="breadcrumb-holder">
        <div class="container-fluid">
            <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a href="{{url('staff')}}">Staff Management</a></li>
    ...
    <section class="charts">
        <div class="container-fluid">
            <div class="charts">
                <div class="container-fluid">
                    <header>
                        <h1 class="h3 display">List of Staff</h1>
                    </header>
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-12 container-fluid">
                <div class="card">
                    <div class="card-body">
                        ...
                        @endif
                        <div style="margin-bottom: 1%;"><a class="btn btn-primary" href="{{ url('staff/create') }}">Add
                                new</a>
                            <div class="pull-right">
                                ...
                                <form class="form-inline mr-auto" action="{{url('staff')}}"
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            @foreach($list as $value)
                                <tr>
                                    ...
                                    <td>{{ $value->name }}</td>
                                    <td>{{$value->tel}}</td>
                                    <td>{{$value->pref['name']}}</td>
                                    <td>{{$value->username}}</td>
                                    <td>{{$value->last_login_time}}</td>
                                    <td>{{$value->created_date}}</td>
                                    <td align="center">
                                        <a class="btn btn-primary"
                                           href="staff/edit/{{$value->id}}"><i
                                                    class="fa fa-pencil-square-o"
                                                    aria-hidden="true"></i></a>
                                    </td>
                                    <td>
                                        <form method="post"
                                              action="{{action('StaffController@delete',$value->id)}}"
                                              class="form-horizontal" id="row-{{$value->id}}"
                                              role="form">
                                            {{ csrf_field() }}
                                            <button type="button" class="btn btn-primary"
                                                    data-myid="{{$value -> id}}"
                                                    data-myname="{{$value -> name}}"
